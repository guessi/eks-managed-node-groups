...
PKGS       := $(shell go list ./...)
REPO       := github.com/guessi/eks-managed-node-groups
...
build-linux-x86_64:
	...
	@CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -ldflags="$(LDFLAGS)" -o ./releases/$(GITVERSION)/Linux-x86_64/eks-managed-node-groups
	@cp ./LICENSE ./releases/$(GITVERSION)/Linux-x86_64/LICENSE
	@tar zcf ./releases/$(GITVERSION)/eks-managed-node-groups-Linux-x86_64.tar.gz -C releases/$(GITVERSION)/Linux-x86_64 eks-managed-node-groups LICENSE

build-linux-arm64:
	@echo "Creating Build for Linux (arm64)..."
	@CGO_ENABLED=0 GOOS=linux GOARCH=arm64 go build -ldflags="$(LDFLAGS)" -o ./releases/$(GITVERSION)/Linux-arm64/eks-managed-node-groups
	@cp ./LICENSE ./releases/$(GITVERSION)/Linux-arm64/LICENSE
	@tar zcf ./releases/$(GITVERSION)/eks-managed-node-groups-Linux-arm64.tar.gz -C releases/$(GITVERSION)/Linux-arm64 eks-managed-node-groups LICENSE

build-darwin-x86_64:
	@echo "Creating Build for macOS (x86_64)..."
	@CGO_ENABLED=0 GOOS=darwin GOARCH=amd64 go build -ldflags="$(LDFLAGS)" -o ./releases/$(GITVERSION)/Darwin-x86_64/eks-managed-node-groups
	@cp ./LICENSE ./releases/$(GITVERSION)/Darwin-x86_64/LICENSE
	@tar zcf ./releases/$(GITVERSION)/eks-managed-node-groups-Darwin-x86_64.tar.gz -C releases/$(GITVERSION)/Darwin-x86_64 eks-managed-node-groups LICENSE

build-darwin-arm64:
	@echo "Creating Build for macOS (arm64)..."
	@CGO_ENABLED=0 GOOS=darwin GOARCH=arm64 go build -ldflags="$(LDFLAGS)" -o ./releases/$(GITVERSION)/Darwin-arm64/eks-managed-node-groups
	@cp ./LICENSE ./releases/$(GITVERSION)/Darwin-arm64/LICENSE
	@tar zcf ./releases/$(GITVERSION)/eks-managed-node-groups-Darwin-arm64.tar.gz -C releases/$(GITVERSION)/Darwin-arm64 eks-managed-node-groups LICENSE

build-windows-x86_64:
	@echo "Creating Build for Windows (x86_64)..."
	@CGO_ENABLED=0 GOOS=windows GOARCH=amd64 go build -ldflags="$(LDFLAGS)" -o ./releases/$(GITVERSION)/Windows-x86_64/eks-managed-node-groups.exe
	@cp ./LICENSE ./releases/$(GITVERSION)/Windows-x86_64/LICENSE.txt
	@tar zcf ./releases/$(GITVERSION)/eks-managed-node-groups-Windows-x86_64.tar.gz -C releases/$(GITVERSION)/Windows-x86_64 eks-managed-node-groups.exe LICENSE.txt
